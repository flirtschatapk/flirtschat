...
export function ProfileImage({position="0% 0%",className="",alt="Flirtschat member",src=null,priority=false}:ProfileImageProps){
  ...
  useEffect(()=>{if(process.env.NODE_ENV==="development"&&requested)console.debug("[PhotoPerf] image-load-start",{sourceType:requested.startsWith("/api/media/profile-photo")?"profile-media":requested.startsWith("/api/")?"private-media":"asset"})},[requested]);
  ...
  const loaded=()=>{if(process.env.NODE_ENV==="development"&&requested)console.debug("[PhotoPerf] image-load-done",{sourceType:requested.startsWith("/api/media/profile-photo")?"profile-media":requested.startsWith("/api/")?"private-media":"asset"})};
  const failedLoad=()=>{if(process.env.NODE_ENV==="development"&&requested)console.info("[PhotoPerf] image-load-failed",{sourceType:requested.startsWith("/api/media/profile-photo")?"profile-media":requested.startsWith("/api/")?"private-media":"asset"});setFailed(true)};
  return <span className={`profile-image ${isSprite?"profile-image--sprite":"profile-image--asset"} ${className}`.trim()}>{requested&&!failed?(isRemote?<img src={requested} alt={alt} loading={priority?"eager":"lazy"} fetchPriority={priority?"high":"auto"} decoding="async" referrerPolicy="no-referrer" style={{objectPosition:effectivePosition,objectFit:"cover"}} onLoad={loaded} onError={failedLoad}/>:<Image src={requested} alt={alt} fill priority={priority} sizes="(max-width: 480px) 50vw, (max-width: 768px) 80vw, 360px" style={{objectPosition:effectivePosition,objectFit:"cover"}} unoptimized={requested.startsWith("/api/")} onLoad={loaded} onError={failedLoad}/>):<UserRound aria-label={alt}/>}</span>;
}
