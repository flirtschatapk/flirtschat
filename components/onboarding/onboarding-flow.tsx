...
export function OnboardingFlow(){
  ...
  useEffect(()=>{const timeout=window.setTimeout(()=>setLoadTrouble(true),8000);void(async()=>{
    if(isSupabaseConfigured()){
      ...
      if(error||!p){setLoadTrouble(true);return}
      ...
      window.clearTimeout(timeout);setReady(true);return;
    }
  ...
  if(!ready)return <main className="route-loading">{loadTrouble?<><span>We&apos;re having trouble loading your account.</span><button className="onboarding-next" type="button" onClick={()=>window.location.reload()}>Retry</button></>:<><LoaderCircle className="spin"/><span>Loading your profile…</span></>}</main>;
