...
type CropQueueItem={file:File;primary:boolean};
...
export function PhotoUploadStep({form}:{form:UseFormReturn<OnboardingValues>}){
  ...
  const[progress,setProgress]=useState<Record<string,number>>({}),[toast,setToast]=useState<UploadToastKind|null>(null),[pending,setPending]=useState<PendingUpload|null>(null),[cropQueue,setCropQueue]=useState<CropQueueItem[]>([]);
  ...
  const runUpload=async(file:File,preview?:string)=>{
    if(!navigator.onLine){if(!preview)setPending({file,preview:URL.createObjectURL(file)});setToast("offline");return false}
    ...
    try{
      ...
      if(preview)URL.revokeObjectURL(preview);setPending(null);setToast("success");return true;
    }catch(error){console.error("Photo upload failed",error);if(!preview)setPending({file,preview:URL.createObjectURL(file)});setToast(navigator.onLine?"failed":"offline");return false}
  ...
  const add=(files:FileList|null)=>{if(!files)return;const next=Array.from(files).slice(0,6-photos.length).filter(file=>{if(!supported.has(file.type)||file.size===0){setToast("format");return false}if(file.size>5*1024*1024){setToast("large");return false}return true}).map((file,index)=>({file,primary:photos.length===0&&index===0}));setCropQueue(next);if(input.current)input.current.value=""};
  const cropSaved=async(file:File)=>{const current=cropQueue[0];if(!current)return;const uploaded=await runUpload(file);if(uploaded)setCropQueue(queue=>queue.slice(1));else setCropQueue([])};
  const retry=()=>{if(pending)void runUpload(pending.file,pending.preview)};
  const remove=(id:string)=>form.setValue("photos",photos.filter(p=>p.id!==id),{shouldValidate:true,shouldDirty:true});
