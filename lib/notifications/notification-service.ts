...
export function loadNotifications(){if(typeof window==="undefined")return mockNotifications;try{const saved=JSON.parse(localStorage.getItem(STORAGE_KEY)??"null")as FlirtschatNotification[]|null;return Array.isArray(saved)?saved:mockNotifications}catch{return mockNotifications}}
export function saveNotifications(items:FlirtschatNotification[]){if(typeof window!=="undefined")localStorage.setItem(STORAGE_KEY,JSON.stringify(items))}
