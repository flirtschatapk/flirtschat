...
export async function getMessageActions(messageIds: string[]): Promise<{ reactions: Map<string, MessageReaction[]>; pins: Set<string> }> {
  const supabase = createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return { reactions: new Map(), pins: new Set() };
  return loadMessageActions(supabase, messageIds, user.id);
}
...
export async function getConversations(selectedConversationId?: string): Promise<Conversation[]> {
  const supabase = createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return [];
  ...
  const actions = { reactions: new Map<string, MessageReaction[]>(), pins: new Set<string>() };
...
export async function sendMessage(conversation: Conversation, message: ChatMessage) {
  ...
  const { data, error } = await supabase.from("fc_messages").insert({ conversation_id: conversation.id, sender_id: user.id, body: message.type === "voice" ? "Sent you a voice message" : message.text, kind: message.type === "photo" ? "image" : message.type === "video" ? "video" : message.type === "voice" ? "voice" : "text", media_path: message.mediaKey || (message.type === "voice" || message.type === "photo" || message.type === "video" ? null : message.mediaUrl) || null, media_mime_type: message.type === "voice" ? canonicalVoiceMime(message.mediaMimeType) : message.mediaMimeType || null, media_size_bytes: message.mediaSizeBytes || null, media_duration_seconds: message.duration || null, reply_to: message.replyTo || null }).select("id,created_at,media_path,media_mime_type,media_size_bytes,media_duration_seconds").single();
  if (error) {
    if (message.type === "voice") {
  ...
  const secureMediaUrl = message.type === "voice" && data.media_path
    ...
    : message.type === "photo" && data.media_path
      ? `/api/media/chat-image?messageId=${encodeURIComponent(data.id)}`
      : message.type === "video" && data.media_path
        ? `/api/media/chat-video?messageId=${encodeURIComponent(data.id)}`
        : message.mediaUrl;
