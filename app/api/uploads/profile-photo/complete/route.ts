...
export async function POST(request:Request){
  ...
  if(body.replacePhotoId){
    ...
    const{data,error}=await supabase.from("fc_profile_photos").update({object_key:body.objectKey,moderation_status:"pending",reviewed_at:null}).eq("id",existingPhoto.id).eq("user_id",user.id).select("id").single();
  ...
  const{data,error}=await supabase.from("fc_profile_photos").upsert({user_id:user.id,object_key:body.objectKey,position,moderation_status:"pending"},{onConflict:"object_key"}).select("id").single();
