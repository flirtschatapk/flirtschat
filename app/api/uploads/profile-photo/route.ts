...
import {getPublicBucket,getPublicR2Url,getR2Client} from "@/lib/r2/server";
...
export async function POST(request:Request){
  ...
  try{
    const command=new PutObjectCommand({Bucket:getPublicBucket(),Key:objectKey,ContentType:body.contentType,ContentLength:body.size});
    const uploadUrl=await getSignedUrl(getR2Client(),command,{expiresIn:300});
    const mediaUrl=getPublicR2Url(objectKey);
