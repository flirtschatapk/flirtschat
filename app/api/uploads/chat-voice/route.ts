...
const allowed=new Set(["audio/webm","audio/webm;codecs=opus","audio/ogg","audio/ogg;codecs=opus"]);
const extension=(contentType:string)=>contentType.startsWith("audio/ogg")?"ogg":"webm";
...
export async function POST(request:Request){
  ...
  if(!body?.conversationId||!body.contentType||!allowed.has(body.contentType))return NextResponse.json({error:{code:"INVALID_AUDIO",message:"Unsupported audio format"}},{status:400});
