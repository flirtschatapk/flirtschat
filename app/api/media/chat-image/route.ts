...
const imageKeyPattern=/^chat-image\/[0-9a-f-]{36}\/[0-9a-f-]{36}\/[0-9a-f-]{36}\.(jpg|jpeg|png|webp)$/i;
...
export async function GET(request:Request){
  ...
  if(messageError||!message||message.deleted_at||message.kind!=="image"||typeof message.media_path!=="string"||!imageKeyPattern.test(message.media_path))return unavailable();
  ...
  try{
    ...
    console.error("Chat image media signing failed",{name:error instanceof Error?error.name:"UnknownError"});
